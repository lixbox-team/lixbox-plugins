...
withCredentials([usernamePassword(credentialsId: 'e1529c62-f3ec-4b12-bbad-2a352fda9af2', usernameVariable: 'JENKINS_LOGIN', passwordVariable: 'JENKINS_PWD')]) {
	node('slave-gradle-jdk11') {    
	    ...
	    stage('Code review & report'){
	        ...
	        try{
	            sh 'export SOURCE_BUILD_NUMBER=${BUILD_NUMBER} && ${WORKSPACE}/gradlew  -Djenkins.login=${JENKINS_LOGIN} -Djenkins.password=${JENKINS_PWD}  -x test --stacktrace sonarqube'
	        }
	        catch (e){
	            onFailed(e);
	            error e
	        ...
	        retry(2){ 
	            try{
	            	withCredentials([usernamePassword(credentialsId: 'f33ba02a-2b78-47ae-9577-86299b22dbde', usernameVariable: 'MVCT_LOGIN', passwordVariable: 'MVCT_PWD')]) {
	                    sh 'export SOURCE_BUILD_NUMBER=${BUILD_NUMBER} && ${WORKSPACE}/gradlew -Djenkins.login=${JENKINS_LOGIN} -Djenkins.password=${JENKINS_PWD}  -Dmavencentral.login=${MVCT_LOGIN} -Dmavencentral.password=${MVCT_PWD} -Dorg.ajoberstar.grgit.auth.username=${GIT_TOKEN} -Penv=prod --stacktrace publish'
                    }
